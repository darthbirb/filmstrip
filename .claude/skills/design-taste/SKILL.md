...
> ## PROJECT SCOPE — read this before anything below
>
> **Filmstrip is a dense desktop utility for Windows, not a landing page or a portfolio.**
> Someone sits in it for hours repeating a handful of actions. Most of what follows was
> written for pages people visit once and admire.
>
> **Take:** motion, micro-interaction, interaction states, small visual craft.
> `reference/motion.md` and `reference/interaction-states.md` are the parts that transfer.
>
> **Leave:** flow, information architecture, layout language, and every marketing pattern.
>
> **Never, in this app:**
> - **No remote assets.** No `picsum.photos`, no CDN script, no hosted font. The app is
>   offline and its Content-Security-Policy forbids it.
> - **No placeholder or sample data.** A surface shows only what it knows.
> - **No new dependency, and never `npx shadcn add`.** Versions are pinned and dependencies
>   are added only when the user agrees.
>
> **Sizes come from the token layer** in `src/styles/app.css`. A component may look however
> it looks, but it may not invent a number. Where a rule below wants a value the scale has no
> name for, the scale gains the name — say so out loud rather than inlining it.
>
> **Motion is filtered by frequency.** The more often an action happens, the less it may
> perform: expressive where an action is rare and deliberate, 120ms or nothing where it is
> reflexive and repeated.
>
> **This skill proposes; it does not decide.** Its output is options for the user to choose
> between, implemented by hand afterwards.
